...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            {
                // Do not rotate player based on input, keep facing target (optional: use LookAt)
                // Set animator parameters for 2D movement
                if (_hasAnimator)
                {
                    _animator.SetFloat("Horizontal", _input.move.x, 0.1f, Time.deltaTime);
                    _animator.SetFloat("Vertical", _input.move.y, 0.1f, Time.deltaTime);
                    _animator.SetFloat(_animIDSpeed, _animationBlend);
                    _animator.SetFloat(_animIDMotionSpeed, inputMagnitude);
                }

                // Move in local space (relative to player orientation)
            ...
            {
                // Default: movement relative to camera
                ...
                if (_hasAnimator)
                {
                    _animator.SetFloat(_animIDSpeed, _animationBlend);
                    _animator.SetFloat(_animIDMotionSpeed, inputMagnitude);
                    // Optionally reset lock-on parameters
        ...
        // Call this method to lock or unlock camera input for target lock (enemy lock-on)
        public void SetCameraInputLock(bool isLocked)
        {
            LockCameraPosition = isLocked;
            if(LockCameraPosition)
            {
